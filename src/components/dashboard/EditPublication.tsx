...
const EditPublication = ({ id }: { id: string }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getSinglePublication(id);
                reset(data);
            } catch (error) {
                toast.error("Failed to fetch publication data.");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div>
            <Card className="w-full max-w-4xl mx-auto">
                <CardHeader>
                    <CardTitle className="text-2xl font-bold text-center">Edit Publication</CardTitle>
                </CardHeader>
                <CardContent>
